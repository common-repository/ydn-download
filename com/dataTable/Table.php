...
use ycd\Subscription;
require_once(dirname(__FILE__).'/ListTable.php');

class YCDTable extends YCDListTable
{
	protected $id = '';
	...
	public function prepare_items()
	{
		...
		/*Remove download data when its class does not exist.*/
	...
	public function __toString()
	{
		...
		<p class="search-box">
			 <input type="hidden" name="post_type" value="<?php echo YCD_COUNTDOWN_POST_TYPE; ?>" >
	...
	{
		$countdownSubscriptions = Subscription::getAllSubscriptionForms();
		...
		$selectedDate = '';
		$list = '';
		if (isset($_GET['ycd-subscription-id'])) {
			$selectedCountdown = (int)$_GET['ycd-subscription-id'];
		}
		if (isset($_GET['ycd-subscribers-date'])) {
			$selectedDate = esc_sql($_GET['ycd-subscribers-dates']);
		}
		?>
        <div class="alignleft actions daterangeactions">
            <select name="ycd-bulk-action">
                <option value=""><?php _e('Bulk Actions', YDN_TEXT_DOMAIN) ?></option>
                <option value="delete"><?php _e('Delete', YDN_TEXT_DOMAIN) ?></option>
            </select>
            <input name="apply_action" id="post-query-submit" class="button ycd-delete-subscribers" value="<?php _e('Apply', YDN_TEXT_DOMAIN)?>" type="submit">
        </div>
        <form method="get" action="/">
            <input name="_wp_http_referer" value="" type="hidden">
            <div class="alignleft actions daterangeactions">
                <input name="filter_action" id="post-query-submit" class="button" value="<?php _e('Filter', YDN_TEXT_DOMAIN)?>" type="submit">
            </div>
        </form>
		<?php
	}
